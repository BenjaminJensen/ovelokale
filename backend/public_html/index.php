...
match ($path) {
    ...
    '/api/recurring-slots' => (function () {
        ...
        $body = json_decode((string) file_get_contents('php://input'), true);
        $result = App\Http\RecurringSlotController::store(is_array($body) ? $body : []);
        http_response_code($result['status']);
        echo json_encode($result['body']);
    })(),

    '/api/recurring-slots/conflicts' => (function () {
        $result = App\Http\RecurringSlotController::conflicts($_GET);
        http_response_code($result['status']);
        echo json_encode($result['body']);
    })(),
